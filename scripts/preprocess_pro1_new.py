...
def run_preprocess(
    ...
    print(f"{total}장 pro1 전처리 시작 (template registration / no-virtual-point)")
...
def main():
    parser = argparse.ArgumentParser(
        description="pro1 전처리(new): 1) Gray -> 2) Template Registration(중심+회전) -> 3) Crop"
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
    ...
    parser.add_argument(
        "--template",
        type=Path,
        default=Path("/home/hjj747/catheter/data/raw/label_orign/pro_1_endpoint.png"),
        help="기준 템플릿 이미지",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("/home/hjj747/catheter/data/processed/processed_images/pro1_new"),
        help="정렬+크롭 결과 저장 폴더",
    )
    parser.add_argument(
        "--overlay-dir",
        type=Path,
        default=Path("/home/hjj747/catheter/data/processed/overlay_images/pro1_new"),
