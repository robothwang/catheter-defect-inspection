...
def make_default_split_path(seed: int) -> Path:
    return Path("/home/hjj747/catheter-preprocessing/experiments/splits") / f"inceptionv3_3class_seed{seed}.csv"
...
def compute_loss(output, labels, criterion, aux_loss_weight: float):
    logits = get_primary_logits(output)
    loss = criterion(logits, labels)
    aux_logits = getattr(output, "aux_logits", None)
...
def train_one_epoch(model, loader, criterion, optimizer, device, aux_loss_weight: float):
    model.train()
    total_loss = 0.0
    total_correct = 0
    total_count = 0
    ...
    for images, labels, _, _ in tqdm(loader, desc="train", leave=False):
        ...
        output = model(images)
        loss, logits = compute_loss(output, labels, criterion, aux_loss_weight=aux_loss_weight)
        loss.backward()
        optimizer.step()
...
def main():
    parser = argparse.ArgumentParser(description="Inception-V3 fine-tuning for catheter 3-class classification")
    ...
    output_dir = args.output_dir or (
        Path("/home/hjj747/catheter-preprocessing/experiments") / f"inceptionv3_{args.dataset_type}"
    )
    ...
    model, weights = build_inception_v3(num_classes=len(CLASS_NAMES), pretrained=True)
