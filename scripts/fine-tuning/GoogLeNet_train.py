...
def make_default_split_path(seed: int) -> Path:
    return Path("/home/hjj747/catheter-preprocessing/experiments/splits") / f"googlenet_3class_seed{seed}.csv"
...
def main():
    parser = argparse.ArgumentParser(description="GoogLeNet fine-tuning for catheter 3-class classification")
    ...
    output_dir = args.output_dir or (
        Path("/home/hjj747/catheter-preprocessing/experiments") / f"googlenet_{args.dataset_type}"
    )
    ...
    model, weights = build_googlenet(num_classes=len(CLASS_NAMES), pretrained=True)
