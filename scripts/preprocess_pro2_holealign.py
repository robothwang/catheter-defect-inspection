...
def _augment_components_if_merged_for_3(components, shape, section_area, min_ratio, max_ratio, min_circularity):
    # 2개로 검출되면 큰 성분 분할을 시도해 3개를 복원한다.
    if len(components) != 2:
        return components
...
def _build_hole_group_masks_3(shape, components):
    # 면적 상위 3개를 사용해 big(1개)/small(2개) 마스크를 만든다.
...
def detect_template_holes(template_gray, template_section_mask):
    ...
    for thr in [245, 242, 240, 238, 235, 232, 228]:
        ...
        comps = _augment_components_if_merged_for_3(
            components=comps,
        ...
        grouped = _build_hole_group_masks_3(template_gray.shape, comps)
        if grouped is None:
            if best is None or len(comps) > best["count"]:
                best = {"count": len(comps), "thr": thr, "grouped": None}
            continue

        gap = grouped["big"]["area"] / max(grouped["small_mean"], 1.0)
        score = 1000.0 + gap
...
def detect_target_holes(target_gray, target_section_mask):
    # 타깃 전용: Otsu 기반 dark-threshold + 완화 파라미터로 3개 lumen을 찾는다.
    ...
    for ksize in [31, 25, 35]:
        ...
        for offset in [0, 5, -5, 10, -10]:
            ...
            grouped = _build_hole_group_masks_3(target_gray.shape, comps)
            count_top = min(len(comps), 3)
            if count_top == 0:
                continue
            ...
            # 3개 검출을 최우선으로, 그다음 작은 홀까지 안정적으로 남는 후보를 선호한다.
...
def find_best_rotation_with_hole_groups(target_grouped, template_grouped, center, coarse_step=3.0, fine_step=0.25):
    # big/small(2개)/all(3개) 마스크를 분리해 회전 점수를 계산한다.
    ...
    def score_at(angle):
        ...
        score = 0.60 * s_big + 0.25 * s_small + 0.15 * s_all
...
def run_preprocess(
    ...
    print(f"{total}장 pro2.1 전처리 시작 (template registration + 3-lumen split)")
...
def main():
    parser = argparse.ArgumentParser(
        description="pro2.1 전처리: 1) Gray -> 2) Template Registration(3-lumen 중심+회전) -> 3) Crop"
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("/home/hjj747/catheter-defect-inspection/data/processed/processed_images/pro2_holealign"),
        help="정렬+크롭 결과 저장 폴더",
    )
    parser.add_argument(
        "--overlay-dir",
        type=Path,
        default=Path("/home/hjj747/catheter-defect-inspection/data/processed/overlay_images/pro2_holealign"),
